import {
	Dialog,
	DialogContent,
	DialogDescription,
	DialogHeader,
	DialogTitle,
} from "@ryu/ui/components/dialog.tsx";
// The Whiteboard app — an Excalidraw board that IS a Space document.
//
// Mirrors the persistence + AI-generate behaviour of the built-in
// `SpaceWhiteboardEditorPage` it replaces, but reaches the platform through the
// capability-gated `window.ryu` bridge instead of the desktop's node client:
//   - Load:  window.ryu.spaces.getDoc({ doc_id })  → parse Excalidraw JSON.
//   - Save:  window.ryu.spaces.updateDoc({ doc_id, title, source })  (debounced;
//            Core re-embeds for search + re-resolves [[backlinks]] on every save).
//   - AI:    generateElements(...) → window.ryu.model.complete (Gateway-routed).
// The `{ spaceId, docId }` it operates on is baked into `window.ryu.context` by the
// host when the app is opened as a Space document.

import {
	convertToExcalidrawElements,
	Excalidraw,
	hashElementsVersion,
	serializeAsJSON,
} from "@excalidraw/excalidraw";
import "@excalidraw/excalidraw/index.css";
import type {
	ExcalidrawElement,
	FileId,
} from "@excalidraw/excalidraw/element/types";
import type {
	AppState,
	BinaryFileData,
	DataURL,
	ExcalidrawImperativeAPI,
	ExcalidrawInitialDataState,
} from "@excalidraw/excalidraw/types";
import type {
	RyuCatalogSnapshot,
	RyuRuntimeSelection,
} from "@ryu/app-host/app-bridge";
import {
	openRealtimeResource,
	type RealtimeResourceChannel,
} from "@ryu/app-host/realtime";
import {
	ModelAgentPicker,
	type RyuPickerSelection,
} from "@ryu/blocks/composer/runtime-picker";
import { Button } from "@ryu/ui/components/button.tsx";
import { Input } from "@ryu/ui/components/input.tsx";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { generateElements } from "./generate";
import { ImagePicker, type WhiteboardImageSelection } from "./ImagePicker";

const SAVE_DEBOUNCE_MS = 800;

type SaveState = "idle" | "saving" | "saved" | "error";

const SAVE_LABEL: Record<SaveState, string> = {
	idle: "",
	saving: "Saving…",
	saved: "Saved",
	error: "Could not save",
};

/** Parse a document's stored `source` into Excalidraw initial data. A blank or
 *  unparseable source opens an empty board rather than erroring. */
function parseScene(source: string): ExcalidrawInitialDataState | null {
	if (!source.trim()) {
		return null;
	}
	try {
		const scene = JSON.parse(source) as ExcalidrawInitialDataState;
		return {
			elements: scene.elements ?? [],
			appState: scene.appState ?? {},
			files: scene.files ?? {},
		};
	} catch {
		return null;
	}
}

export function Whiteboard() {
	const ctx = typeof window === "undefined" ? null : window.ryu?.context;
	const docId = ctx?.docId ?? "";

	const [loaded, setLoaded] = useState(false);
	const [error, setError] = useState<string | null>(null);
	const [title, setTitle] = useState("");
	const [initialData, setInitialData] =
		useState<ExcalidrawInitialDataState | null>(null);
	const [saveState, setSaveState] = useState<SaveState>("idle");
	const [aiOpen, setAiOpen] = useState(false);
	const [aiPrompt, setAiPrompt] = useState("");
	const [aiBusy, setAiBusy] = useState(false);
	const [aiError, setAiError] = useState<string | null>(null);
	const [imageOpen, setImageOpen] = useState(false);
	const [reloadNonce, setReloadNonce] = useState(0);
	const [runtimeCatalog, setRuntimeCatalog] =
		useState<RyuCatalogSnapshot | null>(null);
	const [runtimeSelection, setRuntimeSelection] = useState<
		RyuRuntimeSelection | undefined
	>();

	const apiRef = useRef<ExcalidrawImperativeAPI | null>(null);
	const titleRef = useRef("");
	const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
	const realtimeRef = useRef<RealtimeResourceChannel | null>(null);
	// The scene version at the last successful save — skips no-op PUTs on pure
	// selection/scroll changes (which still fire Excalidraw's onChange).
	const savedVersionRef = useRef<number>(-1);

	useEffect(() => {
		let cancelled = false;
		window.ryu?.catalog
			.snapshot()
			.then((catalog) => {
				if (!cancelled) {
					setRuntimeCatalog(catalog);
				}
			})
			.catch(() => undefined);
		return () => {
			cancelled = true;
		};
	}, []);

	useEffect(() => {
		if (!docId) {
			setError(
				"This whiteboard has no document context. Open it from a Space."
			);
			return;
		}
		const spaces = window.ryu?.spaces;
		if (!spaces) {
			setError("The Spaces capability is not available for this app.");
			return;
		}
		let cancelled = false;
		setLoaded(false);
		setError(null);
		spaces
			.getDoc({ doc_id: docId })
			.then((doc) => {
				if (cancelled) {
					return;
				}
				if (!doc) {
					setError("This whiteboard could not be found.");
					return;
				}
				setTitle(doc.title);
				titleRef.current = doc.title;
				setInitialData(parseScene(doc.source));
				setLoaded(true);
			})
			.catch(() => {
				if (!cancelled) {
					setError(
						"We couldn't open this whiteboard. Check your connection and try again."
					);
				}
			});
		return () => {
			cancelled = true;
		};
	}, [docId, reloadNonce]);

	useEffect(() => {
		if (!docId) {
			return;
		}
		let cancelled = false;
		let channel: RealtimeResourceChannel | null = null;
		openRealtimeResource({
			onChanged: async () => {
				if (timerRef.current) {
					// Keep a local pending edit; applying a peer snapshot here would
					// discard work before the debounced save commits it.
					return;
				}
				const doc = await window.ryu?.spaces.getDoc({ doc_id: docId });
				if (!(doc && !cancelled)) {
					return;
				}
				const scene = parseScene(doc.source);
				setTitle(doc.title);
				titleRef.current = doc.title;
				if (scene?.elements) {
					savedVersionRef.current = hashElementsVersion(scene.elements);
					const currentAppState = apiRef.current?.getAppState();
					apiRef.current?.updateScene({
						appState: currentAppState
							? { ...currentAppState, ...scene.appState }
							: undefined,
						elements: scene.elements,
					});
					if (scene.files && Object.keys(scene.files).length > 0) {
						apiRef.current?.addFiles(Object.values(scene.files));
					}
				}
			},
			roomId: docId,
		})
			.then((connected) => {
				if (cancelled) {
					void connected.close();
					return;
				}
				channel = connected;
				realtimeRef.current = connected;
				void connected.publishPresence({ surface: "whiteboard" });
			})
			.catch(() => {
				// Offline boards continue to save through Spaces and reconnect on remount.
			});
		return () => {
			cancelled = true;
			realtimeRef.current = null;
			if (channel) {
				void channel.close();
			}
		};
	}, [docId]);

	const flush = useCallback(async () => {
		const api = apiRef.current;
		const spaces = window.ryu?.spaces;
		if (!(api && spaces && docId)) {
			return;
		}
		setSaveState("saving");
		const elements = api.getSceneElements();
		const appState = api.getAppState();
		const files = api.getFiles();
		const source = serializeAsJSON(elements, appState, files, "local");
		savedVersionRef.current = hashElementsVersion(elements);
		try {
			await spaces.updateDoc({
				doc_id: docId,
				title: titleRef.current,
				source,
			});
			await realtimeRef.current?.publishChanged();
			setSaveState("saved");
		} catch {
			setSaveState("error");
		}
	}, [docId]);

	const scheduleSave = useCallback(() => {
		if (timerRef.current) {
			clearTimeout(timerRef.current);
		}
		setSaveState("saving");
		timerRef.current = setTimeout(() => {
			flush().catch(() => {
				// `flush` reflects failures via saveState; nothing to add here.
			});
		}, SAVE_DEBOUNCE_MS);
	}, [flush]);

	// Excalidraw fires onChange for selection/scroll too; only schedule a save when
	// the actual scene version advances past the last saved one.
	const onChange = useCallback(
		(elements: readonly ExcalidrawElement[], _appState: AppState) => {
			if (hashElementsVersion(elements) !== savedVersionRef.current) {
				scheduleSave();
			}
		},
		[scheduleSave]
	);

	// Drop generated elements onto the board and scroll them into view.
	const addElements = useCallback((incoming: readonly ExcalidrawElement[]) => {
		const api = apiRef.current;
		if (!api || incoming.length === 0) {
			return;
		}
		const existing = api.getSceneElements();
		const next = [...existing, ...incoming];
		api.updateScene({ elements: next });
		api.scrollToContent(incoming as ExcalidrawElement[], {
			fitToContent: true,
			animate: true,
		});
	}, []);

	const insertImage = useCallback(
		(hit: WhiteboardImageSelection) => {
			const api = apiRef.current;
			if (!api) {
				return;
			}
			const fileId = crypto.randomUUID() as FileId;
			const mimeType = hit.url.match(/^data:([^;,]+)/u)?.[1] ?? "image/jpeg";
			const file: BinaryFileData = {
				id: fileId,
				dataURL: hit.url as DataURL,
				mimeType: mimeType as BinaryFileData["mimeType"],
				created: Date.now(),
				lastRetrieved: Date.now(),
			};
			api.addFiles([file]);
			const width = hit.width > 0 ? Math.min(hit.width, 640) : 360;
			const height =
				hit.height > 0
					? Math.round((width / Math.max(hit.width, 1)) * hit.height)
					: 240;
			const state = api.getAppState();
			const zoom = state.zoom?.value ?? 1;
			const centerX = (state.width ?? 800) / 2 / zoom - state.scrollX;
			const centerY = (state.height ?? 600) / 2 / zoom - state.scrollY;
			addElements(
				convertToExcalidrawElements([
					{
						type: "image",
						fileId,
						x: centerX - width / 2,
						y: centerY - height / 2,
						width,
						height,
					},
				])
			);
			setImageOpen(false);
		},
		[addElements]
	);

	// Flush a pending save on unmount so in-flight edits are not lost.
	useEffect(
		() => () => {
			if (timerRef.current) {
				clearTimeout(timerRef.current);
				flush().catch(() => {
					// Surfaced via saveState above.
				});
			}
		},
		[flush]
	);

	const handleTitleChange = useCallback(
		(next: string) => {
			setTitle(next);
			titleRef.current = next;
			scheduleSave();
		},
		[scheduleSave]
	);

	const runGenerate = useCallback(async () => {
		const prompt = aiPrompt.trim();
		if (!prompt) {
			return;
		}
		setAiBusy(true);
		setAiError(null);
		try {
			const scene = apiRef.current?.getSceneElements() ?? [];
			const elements = await generateElements(
				prompt,
				scene.length,
				runtimeSelection
			);
			if (elements.length === 0) {
				setAiError("The model didn't return anything to draw. Try rephrasing.");
				return;
			}
			addElements(elements);
			setAiOpen(false);
			setAiPrompt("");
		} catch {
			setAiError("Generation failed. Please try again.");
		} finally {
			setAiBusy(false);
		}
	}, [aiPrompt, addElements, runtimeSelection]);

	const initialForCanvas = useMemo(
		() => initialData ?? { appState: { viewBackgroundColor: "#ffffff" } },
		[initialData]
	);

	if (error) {
		return (
			<div className="wb-center">
				<div className="wb-empty">
					<div className="wb-empty-title">Could not open whiteboard</div>
					<div className="wb-empty-desc">{error}</div>
					{docId ? (
						<Button
							className="wb-btn"
							onClick={() => {
								setError(null);
								setReloadNonce((n) => n + 1);
							}}
							type="button"
						>
							Try again
						</Button>
					) : null}
				</div>
			</div>
		);
	}

	if (!loaded) {
		return (
			<div className="wb-center">
				<div className="wb-spinner" />
			</div>
		);
	}

	return (
		<div className="wb-root">
			<div className="wb-toolbar">
				<Input
					aria-label="Whiteboard title"
					className="wb-title"
					onChange={(e) => handleTitleChange(e.target.value)}
					placeholder="Untitled"
					value={title}
				/>
				<span className="wb-save">{SAVE_LABEL[saveState]}</span>
				<Button
					className="wb-btn"
					onClick={() => {
						setAiError(null);
						setAiOpen(true);
					}}
					type="button"
				>
					Generate
				</Button>
				<Button
					className="wb-btn"
					onClick={() => setImageOpen(true)}
					type="button"
				>
					Insert image
				</Button>
			</div>
			<div className="wb-canvas">
				<Excalidraw
					excalidrawAPI={(api) => {
						apiRef.current = api;
					}}
					initialData={initialForCanvas}
					onChange={onChange}
				/>
			</div>
			<Dialog onOpenChange={setAiOpen} open={aiOpen}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Generate with AI</DialogTitle>
						<DialogDescription>
							Describe the diagram to add to this whiteboard.
						</DialogDescription>
					</DialogHeader>
					<form
						className="wb-modal-form"
						onSubmit={(e) => {
							e.preventDefault();
							runGenerate().catch(() => undefined);
						}}
					>
						<div className="wb-runtime-picker">
							<div className="wb-runtime-label">Ryu model lane</div>
							<ModelAgentPicker
								ariaLabel="Choose the Whiteboard model"
								catalog={runtimeCatalog}
								mode="model"
								onDiscoverModels={(providerId) =>
									window.ryu?.catalog.models({ providerId }) ??
									Promise.reject(
										new Error("Ryu model discovery is unavailable.")
									)
								}
								onSelectionChange={(selection: RyuPickerSelection) => {
									if (selection.kind === "model") {
										setRuntimeSelection(selection);
									}
								}}
								placeholder="Choose a Ryu model"
								value={runtimeSelection}
							/>
						</div>
						<Input
							aria-label="Diagram description"
							autoFocus
							onChange={(e) => setAiPrompt(e.target.value)}
							placeholder="e.g. a flowchart for user signup with email verification"
							value={aiPrompt}
						/>
						{aiError ? <div className="wb-modal-error">{aiError}</div> : null}
						<div className="wb-modal-actions">
							<Button
								onClick={() => setAiOpen(false)}
								type="button"
								variant="ghost"
							>
								Cancel
							</Button>
							<Button
								disabled={aiBusy || !aiPrompt.trim()}
								type="submit"
								variant="default"
							>
								{aiBusy ? "Generating…" : "Generate"}
							</Button>
						</div>
					</form>
				</DialogContent>
			</Dialog>
			<ImagePicker
				onOpenChange={setImageOpen}
				onSelect={insertImage}
				open={imageOpen}
			/>
		</div>
	);
}
